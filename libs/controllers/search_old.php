...
class Search extends CI_Controller {
    ...
    */
    ...
    public function purchase()
    {
        ...
        //判断当前时间是否大于预设的保护时间
        if(($get['depart_date'] == date("Y-m-d")) && (time() > $site['stop_ticket'] * 60 + $this->timeTran($departTime)))
        {
            echo "<script>alert('It\'s too late,Please choose again'); window.history.back();</script>";
            exit;
        }
    ...
    public function payment(){
        ...
        file_put_contents("log/ok_response_session.txt", "\r\n payment:User:".print_r($data['user'], 1)."\r\n OID:".$o_id, FILE_APPEND);
    ...
    private function do_complete($response)
    {
        ...
        file_put_contents("log/ok_response_session.txt", "\r\n do_complete:User:".print_r($data['user'], 1)."\r\n OID:".$oid."++date:".date("Y-m-d H:i:s"), FILE_APPEND);
    ...
    public function complete(){
        ...
        //file_put_contents("log/ok_response_session.txt", "\r\n complete:User:".print_r($data['user'], 1)."\r\n OID:".$oid."++date:".date("Y-m-d H:i:s"), FILE_APPEND);
